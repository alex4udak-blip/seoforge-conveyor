...
def analyze(sites):
    ...
    score = round(avg*100)
    # риск-вердикт по порогам (numeric, не выдумка): >70 склейка почти точно, 50-70 риск, <40 ок
    if score >= 70:
        risk = "КРИТИЧНО — сайты структурно близнецы, Google склеит в сеть и забанит разом"
    elif score >= 50:
        risk = "ВЫСОКИЙ — заметный общий паттерн, нужна вариативность структуры"
    elif score >= 35:
        risk = "СРЕДНИЙ — есть общая основа, желательно разнообразить"
    else:
        risk = "НИЗКИЙ — структуры достаточно различны"
